...
def download_coverage_data( bbox, output_file = None, wcs_url='https://wcs.geonorge.no/skwms1/wcs.hoyde-dtm-nhm-25832', coverage_id='dtm_25832', resx=2, resy=2, output_crs='EPSG:25832', format_out='GeoTIFF'):
    """
...
def plot_raster_from_file(file_path):
    ...
    plt.imshow(raster_data, cmap='gray')#, extent=transform.bounds)
    plt.show()


def get_map_image(bbox, output_file = None, layer_name = 'ortofoto', link='https://wms.geonorge.no/skwms1/wms.nib?service=WMS&request=GetCapabilities', size=None, format = 'image/png'):
...
def test_wcs():
    ...
    pict_url = 'https://wms.geonorge.no/skwms1/wms.nib?service=WMS&request=GetCapabilities'
    coverage_id = 'dtm_25832'
    layer_name = 'ortofoto'
    ...
    if not os.path.exists(output_file_geotif):
        ...
        download_coverage_data(wcs_url, coverage_id, bbox, output_file_geotif)
    else:
    ...
    if not os.path.exists(output_file_png):
        ...
        image = get_map_image(pict_url,layer_name, bbox)
        image.save(output_file_png)
    else:
